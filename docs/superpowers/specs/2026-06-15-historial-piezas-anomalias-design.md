...
> - **Métrica del baseline = `copies_total`** (bw+color) entre cambios consecutivos de la misma pieza en la misma máquina. El cálculo **respeta las fronteras de reemplazo de máquina** (particiona por "época de reemplazo" = nº de `is_replacement_start` hasta cada cambio), para no restar contadores a través de un reinicio — corregido en mig. `20260616112305` tras la verificación adversarial (TDD: `tests/rls/part-yield-baseline.test.ts`). Simplificación consciente: no distingue aún tóner color (copias color) de tambores/B&N; el agente (Fase 3) puede afinar por unidad. La tabla de fichas sí soporta las 4 unidades.
> - **`v_part_yield_effective` solo combina fichas con `unit='copies_total'`** (la métrica del baseline). ⚠️ Una ficha cargada en otra unidad (`copies_color`, `copies_bw`, `mois`) se guarda en `part_yield_specs` pero **no aparece en la vista efectiva** (descarte silencioso). Relevante para tóner color, cuya ficha natural del fabricante suele venir en copias color. Pendiente para Fase 3: decidir el mapeo pieza→unidad relevante y exponer el rendimiento por unidad correcta.
> - **Sin filtro de muestras mínimas:** `v_part_yield_baseline` promedia con cualquier `samples >= 1` y expone `samples`; el consumidor (Fase 3) debe exigir un mínimo (p.ej. `samples >= 3`) antes de fiarse del baseline.
...
> - **UI** `/admin/anomalies` (cola con semáforos 🔴 Remplacer / 🟡 Surveiller + acciones Résolu/Acquitter/Écarter), enlace en sidebar (Service → Anomalies) + **banner en el dashboard** (`/admin`) con el contador de anomalías abiertas. RLS admin-only (`machine_anomalies`); test `tests/rls/machine-anomalies.test.ts`.
