...
4. ✅ PR-D: FASE 4 + FASE 5 — COMPLETO (falta solo el gate E2E de PR-E):
   ...
   - ✅ **PR-D.2 — cierres por línea** (`return_machine_to_stock`/`terminate_contract`/`replace_contract_machine`: referencia de cierre = última lectura ACTIF de la MISMA línea (`contract_machine_id`) con `reading_date ≤ date_fin`, orden `(reading_date, recorded_at)` DESC; fallback a `start_counter`. Corrige 2 defectos del «último contador global»: cruce de líneas (P0-3) y lectura posterior a `date_fin` que rechazaba cierres válidos. La retirada facturable ya exigía `end_counter`; la regla de oro de Codex (`end_counter ≥ última lectura real en `date_fin``) queda cubierta porque `reading_date ≤ date_fin` incluye la lectura del mismo día. Mig. `20260617180000`. Banco adversarial `tests/rls/close-by-line.test.ts` (5 casos, TDD rojo→verde en BD real).
   - ✅ **PR-D.3 — guards por línea** (`delete_contract` y guard de cambio de cliente en `update_contract_with_lines` cuentan `machine_counters` por `contract_id` O por `contract_machine_id` de las líneas del contrato; cubre las lecturas con `contract_id` NULL que ata Princity). De paso: `delete_contract` comprueba `invoices` (FK RESTRICT → registro contable inmutable, nunca se borra un contrato facturado). **GO de Codex** + 2 P2 defensivos aplicados: `invoice_lines` por línea en ambos guards (snapshot malformado) + lock TOCTOU (`FOR UPDATE` de `contract_machines` en `delete_contract`). Mig. `20260617200000`. Banco `tests/rls/guards-by-line.test.ts` (10 casos, TDD rojo→verde en BD real). 154 RLS + 113 unit + typecheck + build verde. **3 rondas Codex (GO): base + endurecimiento + cierre del P2 deadlock** (`replace_contract_machine` bloquea contrato→línea, alineado con delete/terminate). Commits `48f011b`,`d35ba0c`,`77d0124`.
   - ✅ **FASE 5 (billing_day) — sin cambio de código.** CHECK 1-31 verificado en BD real (`contracts_billing_day_check`, `contract_machines_billing_day_override_check`) y coherente en todas las capas (CHECK + RPCs `create/update_contract_with_lines` + UI `ContractForm`, todas 1-31). La semántica fin de mes la da N7 en `computeInvoiceMonth` (PR-C.1, commit `a27ba5a`), con 6 casos de test (31-may→mayo, 28-feb→feb, 29-feb bisiesto→feb, 30-jun→junio, 30-abr día30→abril, 31-dic→dic) + casos 1-28. El P1 «billing_day permite 29-31 sin semántica» queda resuelto: se permite a propósito (clientes fin de mes) y N7 lo desambigua. Verificado 2026-06-18.
5. ✅ PR-E: gate E2E de la cadena completa (`tests/rls/gate-facturation-e2e.test.ts`: siembra→`buildContractInvoiceDraft`→`emit_contract_invoice`→factura inmutable, casos de flujo §9.1/5/7/11/14 + inmutabilidad) + doc `docs/gate-final-facturacion-cadena-2026-06-18.md` (mapeo de los 14 casos del §9 a su cobertura unit/RPC/E2E) + `architecture.md` consolidado. Infra: alias `@/` + stub `server-only` en `vitest.rls.config.ts`. 159 RLS + 113 unit + typecheck + build verde. Commit `3310122`. **Pendiente: GO de Codex → mergear toda la rama a main.**
