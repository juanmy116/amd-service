...
// sustituya al anterior en el iPhone en vez de apilarse; `kind` viaja en el payload para que
// el service worker decida si el aviso nuevo debe volver a sonar (solo las asignaciones).
...
export function buildPushMessage(c: PushContext): PushMessage {
  ...
  if (c.kind === 'unassigned') {
    ...
    return { title: `Tâche retirée — ${client}`, body, url: '/tech', tag, kind: c.kind }
  }
  ...
  if (c.entityType === 'incident') {
    ...
    return { title: `${urgent}Nouvelle panne — ${where}`, body, url: `/tech/incidents/${c.entityId}`, tag, kind: c.kind }
  }

  // Siempre al planning, NUNCA a /tech/scan/<serie>/maintenance/<id>: esa página cierra la
  // visita como «QR vérifié» sin haber escaneado nada (ver docs/pendientes.md).
  return {
