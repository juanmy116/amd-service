...
-- inmutabilidad. La invoca princity-sync en runInitialImport. Se añade un guard al inicio que
-- aborta si existe CUALQUIER factura (emitida o anulada): ambas son documentos contables que no
-- deben truncarse. El EXISTS sin filtro por status es deliberado — es la postura más conservadora.
...
BEGIN
  IF EXISTS (SELECT 1 FROM public.invoices) THEN
    RAISE EXCEPTION 'wipe_data_tables bloqueado: existen facturas (documentos contables inmutables)';
  END IF;
  TRUNCATE TABLE
