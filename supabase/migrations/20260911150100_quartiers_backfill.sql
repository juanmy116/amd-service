...
UPDATE public.clients SET quartier_code = CASE
  ...
  WHEN adresse ILIKE ANY (ARRAY['%LIBERTE%','%HLM%','%GRAND DAKAR%','%FRONT DE TERRE%','%SODIDA%','%DIEUPPEUL%','%CASTORS%'])
    THEN 'liberte'
  WHEN adresse ILIKE ANY (ARRAY['%PLATEAU%','%PLACE DE L%INDEPENDANCE%','%MOHAMED V%','%DJILY MBAYE%','%PONTY%','%FELIX FAURE%','%CARDINAL%','%THIANDOUM%','%AVENUE PASTEUR%','%RUE VICENS%','%GALANDOU DIOUF%','%BOULEVARD DE LA REPUBLIQUE%','%BLD DE LA LIBERATION%','%RUE DU PORT%','%ABDOU KARIM BOURGI%','%WAGANE DIOUF%'])
    THEN 'plateau'
