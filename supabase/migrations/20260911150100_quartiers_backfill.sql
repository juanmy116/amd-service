...
UPDATE public.clients SET quartier_code = CASE
  ...
  WHEN adresse ILIKE ANY (ARRAY['%POINT E%','%FANN%','%AMITIE%','%BIRAGO DIOP%','%BOULEVARD DE L%EST%'])
    THEN 'point-e'
  -- «SICAP LIBERTE» es Liberté, no Mermoz: va antes que la regla genérica %SICAP%.
  WHEN adresse ILIKE '%SICAP LIBERTE%'
    THEN 'liberte'
  ...
  WHEN adresse ILIKE ANY (ARRAY['%LIBERTE%','%HLM%','%GRAND DAKAR%','%FRONT DE TERRE%','%SODIDA%','%DIEUPPEUL%','%CASTORS%'])
    THEN 'liberte'
  -- «%PLACE DE L%PENDANCE%» y no «PLACE DE L'INDEPENDANCE»: ILIKE distingue acentos y en los
  -- datos reales la plaza aparece como «L´INDEPENDANCE», «LÍNDEPENDANCE» (errata) y podría
  -- aparecer como «L'INDÉPENDANCE». Las tres contienen «PENDANCE».
  WHEN adresse ILIKE ANY (ARRAY['%PLATEAU%','%PLACE DE L%PENDANCE%','%MOHAMED V%','%DJILY MBAYE%','%PONTY%','%FELIX FAURE%','%CARDINAL%','%THIANDOUM%','%AVENUE PASTEUR%','%RUE VICENS%','%GALANDOU DIOUF%','%BOULEVARD DE LA REPUBLIQUE%','%BLD DE LA LIBERATION%','%RUE DU PORT%','%ABDOU KARIM BOURGI%','%WAGANE DIOUF%'])
    THEN 'plateau'
