...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (invErr) { console.error('[invoice xlsx] invoices', invErr); return new Response('Erreur technique', { status: 503 }) }
  ...
  if (!lines || lines.length === 0) return new Response('Facture sans lignes', { status: 409 })
